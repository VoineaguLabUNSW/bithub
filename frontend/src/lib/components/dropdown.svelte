...
<div class='flex w-52 flex-col items-stretch'>
    ...
    {#each Array.from(groups).map(([key, values]) => [key, values.filter(v => key?.toLowerCase().includes(filter.toLowerCase()) || v.toLowerCase().includes(filter.toLowerCase()))]).filter(([_, values]) => values.length) as [key, values]}
        <div class='p-2 text-gray-400'>{key}</div>
        {#each values as v }
            {#key v}
                {#if v.disabled}
                    <button class="block flex w-full text-left rounded pl-4 p-2 bg-gray-100 dark:hover:bg-gray-600">{v}</button>
                {:else if v === $selected}
                ...
                {:else}
                    <button class="block flex w-full text-left rounded pl-4 p-2 hover:bg-gray-100 dark:hover:bg-gray-600 overflow-x-hidden text-ellipsis" on:click={() => {selected.set(v); open=false}}>{v}</button>
                {/if}
            {/key}
        {/each}
    {/each}
