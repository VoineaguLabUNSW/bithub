...
<div class='flex w-52 flex-col items-stretch'>
    ...
    <Dropdown placement='down' headerClass="m-2" containerClass='w-60' bind:open class="overflow-y-auto px-3 pb-3 text-sm h-44 divide-y divide-gray-100">
    <div slot="header">
        ...
        {#each values as v }
            {#key v}
                ...
                {:else if v.id == $selected?.id}
                    <li class="rounded pl-4 p-2 bg-gray-100 dark:bg-gray-600 overflow-x-hidden text-ellipsis" on:click={() => {open=false}}>{v.name}</li>
                {:else}
