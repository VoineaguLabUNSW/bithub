    ...
    $: {
        if($plotlyArgs && $plotContainer) {
            ...
            const modeBarButtons = [[ "autoScale2d", "zoomIn2d", "zoomOut2d", 
                {
                    ...
                    attr: 'sidebar_toggle',
                    click: () => toggleOn.update(v => !v)
                }, {
                    // https://fontawesome.com/icons/arrow-pointer
                    name: `HOVER${renderCounter}`,
                    title: 'Sidebar Hover',
                    ...
                    attr: 'sidebar_hover',
                    click: () => hoverOn.update(v => !v)
                    
                }]];
            
            // N.B. because we are not using .newPlot, we need to force plotly to update button callbacks using renderCounter
            Plotly.react($plotContainer, $plotlyArgs.plotData,
                ...
                document.querySelector('[data-attr="sidebar_hover"]')?.classList.toggle('active', get(hoverOn));
                document.querySelector('[data-attr="sidebar_toggle"]')?.classList.toggle('active', get(toggleOn));
            });
            
            if(firstRenderControls && $controlsEnabled) {
                tick().then(() => { // Wait so that we can use querySelector on change
                    toggleOn.set(false);
                    hoverOn.set(true);
                });
                firstRenderControls = false;
            }
            renderCounter++;
        }
    }
